...
public class SastRepository : ISastRepository
{
    ...
    public async Task<string> CreateIntegrationProject(SastAnalisysEntity project)
    {
        ...
        {
            httpClient.DefaultRequestHeaders.Add("Authorization", "Basic " + Convert.ToBase64String(System.Text.ASCIIEncoding.ASCII.GetBytes($"{user}:{password}")));
    ...
    {
        var project = this._context.Projects.Where(x => x.Id == id).Select(x => new ProjectEntity()
        {
        ...
        try
        {
            ...
            await this.RunAnalysis(project);
        }
        catch (Exception ex)
        {
    ...
    public async Task SyncAnalysis(int id)
    {
        try
        {
            var _project = this._context.Projects.Where(x => x.Id == id).Select(x=>x.Sast).FirstOrDefault();
            ...
            using (var httpClient = new HttpClient { BaseAddress = baseAddress })
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    var content = JsonSerializer.Deserialize<DTos.DtoMeasuresSearchHistory>(contentString);
